...
for num, item in entry.items():
    print(f'{num}: {item}')
print()
phonebook = PhoneBook(os.getcwd() + '/phonebook.json')
user_input = 0
while not (user_input == 5):
    ...
    if user_input == 1:
        ...
        phonebook.add_entry(last_name=input('Enter last name: '), first_name=input('Enter first name: '),
        ...
        print(table)
        print('Введите новое значение поля или оставьте его пустым, чтобы его не изменять: ')
        ...
        param = input('To search, enter last name/first name/patronymic/company/work phone or personal phone: ')
